<x-layouts.admin>
    ...
    <div class="space-y-6">
        ...
        @php
            $shouldShowNeedsInfoEmailCard =
                $invite->status === 'needs_info'
                || $invite->missingInfoItems->where('resolved', false)->isNotEmpty()
                || $invite->documentRequirements->where('status', 'missing')->isNotEmpty()
                || (int) ($invite->follow_up_send_count ?? 0) > 0;
        @endphp

        @if ($shouldShowNeedsInfoEmailCard)
            <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        <h2 class="text-lg font-semibold text-slate-900">
                            Needs Info Follow-up Email
                        </h2>

                        <p class="mt-1 text-sm text-slate-600">
                            Send a Microsoft Graph email asking the applicant to update missing onboarding information or documents.
                        </p>
                    </div>

                    <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold
                        {{ $invite->follow_up_last_error
                            ? 'bg-red-100 text-red-700'
                            : ((int) ($invite->follow_up_send_count ?? 0) > 0
                                ? 'bg-emerald-100 text-emerald-700'
                                : 'bg-slate-100 text-slate-600')
                        }}">
                        @if ($invite->follow_up_last_error)
                            Last send failed
                        @elseif ((int) ($invite->follow_up_send_count ?? 0) > 0)
                            Sent
                        @else
                            Not sent
                        @endif
                ...
                <dl class="mt-5 grid gap-4 sm:grid-cols-3">
                    <div class="rounded-xl bg-slate-50 p-4">
                        ...
                        <dd class="mt-1 text-sm font-medium text-slate-900">
                            {{ $invite->last_follow_up_sent_at?->format('d M Y, g:i A') ?? 'Not sent yet' }}
                        </dd>
                    </div>
                    ...
                    <div class="rounded-xl bg-slate-50 p-4">
                        ...
                        <dd class="mt-1 text-sm font-medium text-slate-900">
                            {{ (int) ($invite->follow_up_send_count ?? 0) }}
                        </dd>
                    </div>
                    ...
                    <div class="rounded-xl bg-slate-50 p-4">
                        ...
                        <dd class="mt-1 text-sm font-medium text-slate-900">
                            {{ $invite->resubmitted_at?->format('d M Y, g:i A') ?? 'Not resubmitted yet' }}
                        </dd>
                    </div>
                </dl>
                ...
                @if ($invite->follow_up_last_error)
                    <div class="mt-5 rounded-xl border border-red-200 bg-red-50 p-4">
                        ...
                        <p class="mt-2 whitespace-pre-wrap break-words text-xs text-red-700">
                            {{ $invite->follow_up_last_error }}
                        </p>
                    </div>
                @endif
                ...
                <div class="mt-5 flex flex-wrap gap-3">
                    <a
                        href="{{ route('admin.onboarding.invites.needs-info-email-preview', $invite) }}"
                        target="_blank"
                    ...
                    >
                        Preview Needs Info Email
                    </a>

                    @if (auth()->user()?->canReview())
                        <form method="POST" action="{{ route('admin.onboarding.invites.send-needs-info-email', $invite) }}">
                            @csrf
                            ...
                            >
                                {{ (int) ($invite->follow_up_send_count ?? 0) > 0 ? 'Resend Follow-up Email' : 'Send Follow-up Email' }}
                            </button>
                        </form>
                    @endif
                </div>
